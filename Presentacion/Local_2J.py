...
def Local_2J(nombre1, nombre2):
    ...
    while not partida.haTerminado:
        ...
        # for ind, tablero in partida.tablerosMovimientos:
        #     print('Tablero',ind)
        #     print(tablero)
...
def procesarComandos(partida, comando1, comando2):
    ...
    for opComando in comando1Splitted:
        ...
        if opElementos[0] == 'mover':
            ...
            # o1 = Disparo(f1,partida.tableroActual.nodos[partida.tableroActual.dondeEsta(f1)], int(opElementos[2]))
            o1 = Disparo(f1,partida.tableroActual.nodos[partida.tableroActual.dondeEsta(f1)], x=opElementos[2], y=opElementos[3])
    ...
    for opComando in comando2Splitted:
        ...
        if opElementos[0] == 'mover':
            if opElementos[1]=='lancero': f2 = Lancero(1)
            elif opElementos[1]=='guerrero': f2 = Guerrero(1)
            elif opElementos[1]=='arquero': f2 = Arquero(1)
            elif opElementos[1]=='barbaro': f2 = Barbaro(1)
            elif opElementos[1]=='caballero': f2 = Caballero(1)
            ...
            # o2 = Disparo(f2,partida.tableroActual.nodos[partida.tableroActual.dondeEsta(f2)], int(opElementos[2]))
            o2 = Disparo(f2,partida.tableroActual.nodos[partida.tableroActual.dondeEsta(f2)], x=opElementos[2], y=opElementos[3])
